...
svg('disney by movies.svg', width = 15, height = 10)
...
p3 <- ggplot() +
  ...
  geom_text(data = disney[disney$Type != "Other",], mapping = aes(x = as.factor(Year), y = Gross, label = Movie.title),
            size = 4, color = "#434A54", family = "Segoe UI Light") +
  ...
  scale_fill_manual(name = "",
                    breaks = c("Star Wars", "Marvel", "Disney Animation", "Pixar", "Live Action Adaptation", "Pirates of the Caribbean", "Other"),
                    values = c("#0080FF", "#FFA500", "#3AB795", "#c6cbd2", "#733988", "#FF748C", "#F7CB15")) +
